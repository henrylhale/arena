    ...
    def train(self) -> Autoencoder:
        ...
        for epoch in range(self.args.epochs):
            ...
            for img, label in pbar:
                ...
                pbar.set_postfix(loss=loss.item(), step=self.step)
...
utils.visualise_output(output, grid_latent, title="Autoencoder latent space visualization", filename=str(section_dir / "visualise_output.html"))
...
# Plot the results
utils.visualise_input(latent_vectors, labels, holdout_latent_vectors, HOLDOUT_DATA, filename=str(section_dir / "visualise_input.html"))
...
class VAETrainer:
    ...
    def loss(self, logits: Tensor, mu: Tensor, logsigma: Tensor, img: Tensor) -> float:

        mse = nn.MSELoss()(logits, img)
        kl = 0.5 * (t.exp(logsigma)**2 + mu**2 - t.ones_like(mu)) - logsigma
        return mse + self.args.beta_kl * t.mean(kl) 
    ...
    def training_step(self, img: Tensor):
        ...
        self.step += 1
        img = img.to(device)
        (logits, mu, logsigma) = self.model(img)
        loss = self.loss(logits, mu, logsigma, img)
        loss.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
        if self.args.use_wandb:
