...
from torchvision.datasets.pcam import download_file_from_google_drive
...
def multiply_back1(grad_out: Arr, out: Arr, x: Arr | float, y: Arr) -> Arr:
    ...
    if not isinstance(x, Arr):
        x = np.array(x)
    return multiply_back0(grad_out=grad_out, out=out, x=y, y=x)
...
class BackwardFuncLookup:
    ...
    def add_back_func(self, forward_fn: Callable, arg_position: int, back_fn: Callable) -> None:
        self.back[forward_fn.__name__][arg_position] = back_fn

    def get_back_func(self, forward_fn: Callable, arg_position: int) -> Callable:
        return self.back[forward_fn.__name__][arg_position]
...
def log_forward(x: Tensor) -> Tensor:
    ...
    out = tensor(array = np.log(x.array), requires_grad=requires_grad)
    if requires_grad:
        out.recipe = Recipe(
            func = np.log,
            args = (x.array,),
...
def multiply_forward(a: Tensor | int, b: Tensor | int) -> Tensor:
    ...
    out = tensor(array = arg_a * arg_b, requires_grad=requires_grad)
    parents = {}
    ...
    for node in graph:
        ...
        if node.recipe is not None:
            for i, parent in node.recipe.parents.items():
                grads[parent] = np.zeros_like(parent.array) if parent not in grads else grads[parent]
                back_func = BACK_FUNCS.get_back_func(node.recipe.func, i)
                grads[parent] = grads[parent] + back_func(grads[node], node.array, *node.recipe.args, **node.recipe.kwargs)
...
def permute_back(grad_out: Arr, out: Arr, x: Arr, axes: tuple) -> Arr:
    ...
    function.
    """
    return np.transpose(grad_out, np.argsort(list(axes)))
...
BACK_FUNCS.add_back_func(np.subtract, 1, lambda grad_out, out, x, y: -1 * unbroadcast(grad_out, y))
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        w = 2 * rw * (np.random.random((out_features, in_features)) - 0.5)
        self.weight = Parameter(Tensor(w, requires_grad=True))
        if bias:
            rb = 1 / np.sqrt(in_features)
            b = 2 * rb * (np.random.random((out_features)) - 0.5)
            self.bias = Parameter(Tensor(b, requires_grad=True))
        else:
            self.bias = None
